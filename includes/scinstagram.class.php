...
$scIncludePath = get_include_path().PATH_SEPARATOR.
						$socialcast->plugin_path.'/includes/instagram-php-api/'.PATH_SEPARATOR.
						$socialcast->plugin_path.'/includes/PowerHour_Geocoder/';
...
function getLatLng($address){
	global $socialcast;		
	require_once( $socialcast->plugin_path . '/includes/PowerHour_Geocoder/Geocoder.php');			
...
class SCInstagram
{
	...
	function SCInstagram()
	{
		...
		add_shortcode('socialcast_instagram', array(&$this, 'shortcode'));
	...
	function shortcode($params)
	{
		...
		$buttonNextHtml = '<a href="'.get_bloginfo( 'wpurl' ).'" class="next-page-sc" rel="%d">'.__('Next', 'socialcast').' &gt;&gt;</a>';
		
		$buttonPrevHtml = '<a href="'.get_bloginfo( 'wpurl' ).'" class="prev-page-sc" rel="%d">&lt;&lt; '.__('Previous', 'socialcast').'</a>';
		...
		{			
			...
			if(!$options['scinstagram_access_token'])
			{
				...
				if($token)
				{
					...
					echo '<div class="updated"><p>'.__('Settings saved.', 'socialcast').'</p></div>';
				}
				else if($errorMessage) 
				{
					echo '<div class="error"><p>'.__('Instagram API reported the following error', 'socialcast').': <b>';
					echo $errorMessage;
					echo '</b></p></div>';
				}
			}
	...
	function register_instagram_menu()
	{
		...
		$this->admin_page = add_submenu_page( 'socialcast', __('Instagram settings', 'socialcast'), __('Instagram settings', 'socialcast'), 'manage_options', $this->token, array($this, 'handleOptions') );
	...
	function getErrors()
	{
		...
		if(!SCInstagram::getInstance()->cacheIsEnabled())
			$errors[] = sprintf(__('To improve performance of this plugin, it is highly recommended to make the directory wp-content or wp-content/cache writable. For further information click <a target="_blank" href="%s">here</a>' , 'socialcast'), 'http://codex.wordpress.org/Changing_File_Permissions');
		if(!SCInstagram::getInstance()->isCurlInstalled())
			$errors[] = __('Instagram requires <a href="http://php.net/manual/en/book.curl.php" target="_blank">PHP cURL</a> extension to work properly', 'socialcast');
		if(!function_exists('mb_detect_encoding'))
			$errors[] = __('Geocoding won\'t work unless <a href="http://www.php.net/manual/en/mbstring.installation.php" target="_blank">mbstring</a> is activated', 'socialcast');
			
		return (count($errors) > 0 ? $errors : false);
	}

    function get_gallery_images( $attr=array() ){
        global $socialcast, $scinstagram, $post;
                ...
                <dl class="gallery-item">
                    <dt class="gallery-icon">
						<a href="'.$obj->images->standard_resolution->url.'" title="'.$obj->caption->text.'" rel="lightbox-" target="_blank">
                            <img src="'.$socialcast->plugin_url.'/includes/timthumb.php?src='.$obj->images->low_resolution->url.'&amp;w='.$width.'&amp;h='.$height.'&amp;zc=1&amp;q=90" alt="Instagram Gallery" class="sc-image" width="'.$width.'">
