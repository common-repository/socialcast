...
class SCFacebook
{
	...
	function register_facebook_menu()
		{
			...
			$this->admin_page = add_submenu_page('socialcast', __( 'FB Album Settings', 'socialcast' ), __( 'FB Album Settings', 'socialcast' ), 'manage_options', $this->token, array( &$this, 'scfacebook_admin_page' ) );
	...
	function scfacebook_admin_page()
	{
		global $scfacebook_name;
			...
			{
					?><div class="error"><p><strong><?php _e('Warning', 'socialcast'); ?> :</strong> <?php _e('Another plugin has included the Facebook API throughout all of Wordpress.  I suggest you contact that plugin\'s author and ask them to include it only in pages where it\'s actually needed.', 'socialcast'); ?><br /><br /><?php _e('Things may work fine as-is, but only if the API version included by the other plugin is at least as recent as the one required by Facebook Photo Fetcher.', 'socialcast'); ?></p></div><?php
			}
			else
			{
      ...
      <div class="clear"></div>
			<div id="main">
				...
				<div class="section">
					<h3 class="heading"><?php _e( 'Connect with Facebook', 'socialcast' ); ?></h3>
					<div class="option">
					<?php
					...
					<form method="post" id="step1Frm" action="">
						<input type="submit" class="button-secondary" id="step1Btn" value="<?php echo _e('Reset Account','socialcast'); ?>" />
    ...
    function get_gallery_images( $attr = array() ){
        ...
        global $socialcast, $scfacebook, $post;
...
function sc_facebook_fetch_album_content($aid, $params)
{
    ...
    global $scfacebook_version, $socialcast;
    ...
    foreach($photos as $photo)
    {
        ...
        $width = '150'; $height = '150'; $alt = __('Facebook Gallery', 'socialcast');
