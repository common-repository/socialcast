...
<div id="sc_options" class="wrap <?php if (get_bloginfo('text_direction') == 'rtl') { echo 'rtl'; } ?> sc_instagram">
	...
	<?php
		...
		if(!$isPHP5)
		{
			$scErrors = array(sprintf(__('SocialCast requires at least PHP 5.0 to work properly, your version is: %s', 'socialcast'), phpversion()));
		}
		else
		{
    ...
    <div class="group c1" id="sc-option-intaaccsettings">
        <h3 class="title"><?php _e( 'Instagram Account Settings', 'socialcast' ); ?></h3>
        ...
        <table class="form-table">
			...
			if(SCInstagram::getInstance()->getAccessToken()):
				...
				_e('Your application is authorized, have fun!', 'socialcast');
				echo '</p>';
			?>
				<p>
					<input type="submit" class="button" name="scinstagram-reset-settings" value="<?php _e('Reset settings', 'socialcast'); ?>" />
				</p>
                        
			<?php else: ?>
            
				<p><?php _e('To activate, just enter your Instagram username and password', 'socialcast') ?>:</p>
                <tr valign="top">
                    <th scope="row"><label for="scinstagram-app-user-username"><?php _e('Username', 'socialcast') ?></label></th>
                ...
                <tr valign="top">
                    <th scope="row"><label for="scinstagram-app-user-password"><?php _e('Password', 'socialcast') ?></label></th>
                    <td>
                ...
                <tr valign="top"><td><p><input type="submit" class="button" name="scinstagram-update-auth-settings" value="<?php _e('Save settings', 'socialcast'); ?>" /></p></td></tr>
    ...
    <div class="group c1" id="sc-option-instageneralsettings">
        <h3 class="title"><?php _e( 'Instagram General settings', 'socialcast' ); ?></h3>
    
        <table class="form-table">
            <tbody>
                <tr valign="top">
					<?php if(SCInstagram::getInstance()->cacheIsEnabled()): // Cache aktiv?	?>
                    <th scope="row"><label for="scinstagram-cache-time"><?php _e('Refresh cache after', 'socialcast') ?></label></th>
                    <td>
						...
						<select id="scinstagram-cache-time" name="scinstagram-cache-time">
							...
							<?php endforeach; ?>
						</select><?php _e('minutes', 'socialcast') ?>
                    </td>

					<?php else: // Cache inactive ?>
						<p><?php _e('Cache is not active', 'socialcast'); ?></p>
					<?php endif; ?>
                </tr>
    
                <tr valign="top">
                    <th scope="row"><label for="scinstagram-disable-fancybox"><?php _e('FancyBox Effect', 'socialcast'); ?></label></th>
                    <td>
						...
						<?php _e('Disable fancybox effect', 'socialcast'); ?></label>
                        <br />
                ...
                <tr valign="top">
                    <th scope="row"><label for="scinstagram-disable-image-attr"><?php _e('Image Attribute', 'socialcast'); ?></label></th>
                    <td>
						...
						<?php _e('Disable width and height attribute for images (e.g. for responsive layouts)', 'socialcast'); ?>
						</label>
                    </td>
                </tr>
        ...
        <p><input type="submit" class="button" name="scinstagram-update-settings" value="<?php _e('Save settings', 'socialcast'); ?>" /></p>
