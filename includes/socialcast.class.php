...
Class SocialCast
{
	...
	function __construct($file){
		...
		$this->name = __( 'SocialCast', 'socialcast' );
		$this->token = 'socialcast';
		...
		add_action( 'media_upload_socialcast', array(&$this, 'media_upload_socialcast') );
	...
	public function load_localisation () {
		...
		load_plugin_textdomain( 'socialcast', false, $lang_dir );
	...
	/**
	 * Adds a "SocialCast" tab to the "Add Media" panel.
	 *
	 * @param $tabs
	 * @return array
	 */
	function media_upload_tabs($tabs) {
		$tabs['socialcast'] = 'SocialCast';
		return $tabs;
	}
    
	/**
	 * Invokes the form to display the socialcast insertion screen in the "Add Media" panel. The call to wp_iframe ensures that the right CSS and JS are called.
	 *
	 * @return void
	 */
	function media_upload_socialcast() {
		wp_iframe(array(&$this, 'media_upload_socialcast_form'));
	}
    
	/**
	 * First prints the standard buttons for media upload, then shows the UI for SocialCast.
	 *
	 * @return void
	 */
	function media_upload_socialcast_form() {
		media_upload_header();
		require_once( $this->plugin_path ."/socialcast-form.php");
	}
	...
	static function get_image_sizes_selection($element_name, $show_full = false) {
		...
		foreach ($standard_sizes as $standard_size) {
			if ($standard_size != 'full') {
				...
				$image_sizes[$standard_size] = array('width' => __('Original width', 'socialcast'), 'height' => __('Original height', 'socialcast'));
			}
		}
	...
	function modify_gallery($content, $attr = array()) {
		global $post, $socialcast, $scinstagram, $scfacebook, $scflickr, $socialcast_default_gallery_type, $socialcast_nested_shortcodes;
		if ($attr == null) {
			$attr = array();
		}

		$attr = array_merge(array(
			// Specially for SocialCast
			'type' => $socialcast_default_gallery_type,  //default, flickr, picasa
			'style' => 'default',   //default
			'id'         => $post->ID,
		), $attr);

		if ($socialcast_nested_shortcodes) {
	...
	function admin_page(){
        ...
        <div id="socialcast" class="wrap">
        
        	<div id="icon-options-general" class="icon32"><br/></div>
            <!--div id="icon-socialcast" class="icon32"><br/></div-->
        	<h2><?php echo esc_html( $this->name ); ?> <span class="version"><?php echo esc_html( $this->version ); ?></span></h2>
            <p class="by-colorlabs"><?php _e( 'Powered by', 'socialcast' ); ?><a href="http://colorlabsproject.com" title="ColorLabs"><img src="<?php echo $this->assets_url; ?>images/colorlabs.png" alt="ColorLabs" /></a></p>
        	...
        	<form method="post" action="options.php" id="options">
                ...
                // Make selects data
                $sc_type_instaArr = array( __('Popular','socialcast'), __('Self','socialcast'), __('Feed','socialcast'));
                
                ?>
                
        		<?php wp_nonce_field('update-options'); settings_fields('sc-options'); ?>            
            
                <?php require_once('socialcast.admin.php'); ?>
